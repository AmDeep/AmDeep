# Hi, I'm Amardeep Singh (AmDeep) 👋

![Profile Views](https://komarev.com/ghpvc/?username=AmDeep&color=brightgreen&style=for-the-badge)

**ML/AI Engineer | Full-Stack Developer** | Toronto, Canada

---

### 🛠️ Technologies & Skills
- **Languages**: Python, JavaScript/TypeScript, SQL
- **ML/AI**: Scikit-learn, TensorFlow/Keras, Azure ML, PINNs, NLP, Predictive Modeling
- **Web**: React, Node.js, Streamlit, NGL Viewer
- **Cloud & Tools**: AWS, Git, Jupyter

---

### 🚀 Featured Projects
- [ML_Nanodegree_Capstone_Project](https://github.com/AmDeep/ML_Nanodegree_Capstone_Project) — NASA Predictive Maintenance
- [CAPSTONE-PROJECT](https://github.com/AmDeep/CAPSTONE-PROJECT) — Marketing Campaign Analysis
- [AI_LaunchLab_Team3](https://github.com/AmDeep/AI_LaunchLab_Team3)
- [Capstone_Project3_AzureML_Nanodegree](https://github.com/AmDeep/Capstone_Project3_AzureML_Nanodegree)
- Biotech & Sustainability projects (Metabolic Modelling, Mini-AlphaFold, Solar RO, etc.)

---

<!-- AUTO-METRICS-START -->
$(cat metrics.tmp)
<!-- AUTO-METRICS-END -->

---

### 🌐 Connect
- **Portfolio**: [amdeep.github.io](https://amdeep.github.io/)
- **LinkedIn** & **Email**: Available upon request

**Open to ML/AI opportunities.**
